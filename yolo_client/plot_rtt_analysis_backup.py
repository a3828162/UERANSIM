...
比較不同調度方法 (Random, RoundRobin, LLM, ShortestPath) 的 RTT 表現
"""
...
class RTTAnalyzer:
    ...
    def _extract_method_name(self, dirname):
        ...
        # 精確匹配縮寫（避免誤判）
        if dirname_lower.startswith('ld_') or dirname_lower.startswith('lowestdelay'):
            return 'LowestDelay'
    ...
    def plot_rtt_boxplot(self):
        ...
        for data, scenario, filename in [
            ...
            plt.tight_layout()
        output_file = self.output_dir / "rtt_distribution_boxplot.png"
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
    ...
    def plot_rtt_scalability(self):
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        fig.suptitle(f'{self.condition.upper()} - RTT Scalability Analysis', 
        ...
        ax.set_title('RTT Performance Across Scenarios', fontsize=13, fontweight='bold')
    ...
    def plot_rtt_delta(self):
        ...
        # 繪製
        fig, ax = plt.subplots(figsize=(10, 6))
        fig.suptitle(f'{self.condition.upper()} - RTT Change: Overloaded - Satisfied (Δ)', 
                    fontsize=16, fontweight='bold')
        ...
        ax.set_title('RTT Increase/Decrease', fontsize=13, fontweight='bold')
        ax.grid(axis='y', alpha=0.3)
    ...
    def generate_all_plots(self):
        ...
        print("\n生成的圖表:")
        print("  ├── rtt_method_comparison.png")
        print("  ├── rtt_distribution_boxplot.png")
        print("  ├── rtt_heatmap.png")
...
範例:
  ...
  python3 plot_rtt_analysis.py --condition delay --exclude ShortestPath
    ...
    parser.add_argument(
        '--exclude',
        nargs='+',
        choices=['Random', 'RoundRobin', 'LLM', 'ShortestPath', 'LowestDelay'],
        default=[],
        help='要排除的方法 (可指定多個)'
    )
